...
ccl_device_inline float3 fisheye_lens_opencv_to_direction(
    float u, float v, float coeff0, float4 coeffs, float fov, float width, float height, float fx, float fy, float cx, float cy)
{
  u = (u - cx) / fx;
  v = (v - cy) / fy;
  ...
  if ( converged && !theta_flipped)
  {
      ...
      float phi = safe_acosf((thetad != 0.0f) ? u / thetad : 0.0f);
